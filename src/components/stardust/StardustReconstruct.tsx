/**
 * StardustReconstruct - 星尘重建（照片 → 2D 星尘画像）
 *
 * 上传宠物照片 → Canvas 真实星尘化滤镜 → 产出 2D 星尘画像。
...
export default function StardustReconstruct({ isVip, stardustCoins, onSpendCoins, triggerToast }: StardustReconstructProps) {
  ...
  const handleGenerate = async () => {
    ...
    if (needPayForQuota) {
      ...
      if (stardustCoins < totalCost) {
        triggerToast(`⚠️ 星尘币不足，本次生成需 ${totalCost} 星尘币。`);
        playSound("beep");
        return;
      }
      const ok = window.confirm(`💎 今日免费次数已用完，本次生成需支付 ${totalCost} 星尘币，是否继续？`);
      if (!ok) return;
      if (!payAndGenerate()) {
      ...
      if (isPaidStyle && !onSpendCoins(PAY_PER_GENERATE)) {
        triggerToast("⚠️ 星尘币不足，无法使用赛博星云风。");
        return;
      }
    } else if (isPaidStyle) {
      // 有免费次数，但赛博风格是付费风格，额外扣费
      if (stardustCoins < PAY_PER_GENERATE) {
        triggerToast(`⚠️ 赛博星云风需 ${PAY_PER_GENERATE} 星尘币。`);
        playSound("beep");
        return;
      }
      const ok = window.confirm(`💎 赛博星云风为付费风格，需支付 ${PAY_PER_GENERATE} 星尘币，是否继续？`);
      if (!ok) return;
  ...
  const handleSave = () => {
    ...
    a.download = `星尘画像_${result.id}.png`;
          ...
          >
            ...
            <div className="space-y-2">
              ...
              <p className="text-center text-[10px] text-purple-400/70">
                当前余额：<span className="text-amber-300 font-mono">{stardustCoins}</span> 星尘币
              </p>
            </div>
          </motion.div>
        )}
      ...
      {history.length > 0 && (
        <div>
          <div className="flex items-center justify-between mb-3">
            <h5 className="text-sm font-medium text-purple-200">我的星尘画像</h5>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            <div className="text-sm font-medium text-white">
              3D 星尘建模 <span className="text-[10px] px-1.5 py-0.5 rounded bg-amber-500/80 ml-1">即将上线</span>
            </div>
